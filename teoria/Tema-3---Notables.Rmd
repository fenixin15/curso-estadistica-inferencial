...
* Supongamos que la probabilidad de éxito es  $P(E)=p$, por lo que  $P(F)=1-p=q$ con $0<p<1$.
...
* Fue  [Jacob Bernoulli](https://es.wikipedia.org/wiki/Jakob_Bernoulli)  científico suizo y una más de la de la [familia de científicos suizos Bernoulli](https://es.wikipedia.org/wiki/Familia_Bernoulli)
...
## Función de probabibildad de la distribución binomial
...
para distintos valores de $n$ y $p$ para que disfruteis de tan anscentral método de cálculo. 
...
EL ejemplo anterior correspondería a repetior 100 veces el experimento lanzar una moneda  20 veces y contar el número de caras.
</div>
...
```
</div class="observ"> Notemos que la secuencia aletoria generada no es la misma que con R. De hecho volvemos a ejecutarla obtendremos una muestra alearoria distinta.
</div>
...
## Gráficos de la distibución binomial con python

</div class="exercise">
Ejercicio
Buscad en los manuales de python cómo se dibuja la función de probablidad y de distribución de una binomial.
</div>
...
## Gráficos de la distibución binomial con python (GGGGRANDES)

```{python dibu_python2,echo=FALSE,fig.width=3,fig.eight=3}
n, p = 5, 0.4
x = np.arange(binom.ppf(0.01, n, p),binom.ppf(0.99, n, p))
plt.plot(x, binom.pmf(x, n, p), 'bo', ms=8, label='binom pmf')
plt.vlines(x, 0, binom.pmf(x, n, p), colors='b', lw=5, alpha=0.5)
...
Ahora, al  ser los intentos sucesos independientes, podemos hacer:

$$
\begin{eqnarray*}
P(X>3) & = & P(\{FFFF\})= P(F)\cdot P(F)\cdot P(F)\cdot P(F)\\
&=& (1-p)\cdot (1-p)\cdot (1-p)\cdot (1-p)\\
&=& (1-p)^{3+1}=(1-p)^{4}.
\end{eqnarray*}
$$
...
De la misma forma podemos generalizar a cualquier entero positivo $k$ 

$$F_X(k)=P(X\leq k)=1-(1-p)^{k+1}$$
...
\frac{P\left(X>k+j \right)}{P\left(X\geq j \right)}\\
&=& \frac{1-P(X\leq k+j)}{1-P(X\leq j-1)} = \\
&=& \frac{1-(1-(1-p)^{k+j+1})}{1-(1-(1-p)^{j-1+1})}=\frac{(1-p)^{k+j+1}}{(1-p)^{j}}\\
&=&(1-p)^{k+1}
\end{eqnarray*}
$$
...
la igualdad anterior significa  que aunque **ya llevemos más de $j$ fracasos** la probabilidad de **que fracasemos $k$ veces más** no disminuye; es la misma  que si empezáramos de nuevo el experimento. 
...
Es decir después de 5 fracasos no estamos más cerca de abrir la puerta, ya que  por la propiedad de la  falta de  memoria  es como si empezásemos  de nuevo a abrir la puerta. después de más de 5 fallos la probabilidad de que fallemos al menos 4 veces más es la misma.
</div>

## La variable geométrica que cuentao los intentos para obtener el primer éxito.
...
**Número de partidos consecutivos ganados Madrid-Barça**
...
Sea $Y$ la variable que cuenta el número de veces que  en un partido de futbol de la liga el Real Madrid  pierde contra el Barça sea en el Camp Nou  o el Calderón.

Un aficionado culé (Barça) o blanco (Madrid)  quiere averiguar cuántos partidos de *EL Clásico** tiene que ver hasta ver ganar al Barça

Para ello  consulta [datos historicos de **EL clásico**  en la wikipedia](https://es.wikipedia.org/wiki/El_Cl%C3%A1sico)  y  averigua que hasta el 3 de marzo de 2019  el Real Madrid ganó en 72 ocasiones el Barça en 72  y empataron 34 veces, en totalse  han jugado 178 **Clásicos**.

Como nuestro aficionado tiene un mamá estadística  averigua que  probabilidad de que el barca gane es $P(Barça)=\frac{72}{178}=`r 72/178`$.

Supongamos  que el experimento sigue una ley  geométrica con probailidad de éxito .  $p=P(Barça)=\frac{72}{178}$

Es decir $Y=X+1$ donde $X$ es una v.a que sigue una ley $Ge(p=0.1)$. En este caso el "éxito" es que no nos de el dinero.
