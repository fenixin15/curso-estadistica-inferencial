...
Y por ejemplo con un equipo de fútbol profesional  que tiene en plantilla 22 jugadores  (quitamos el portero) se pueden formar ¡¡nada menos que!!
...
TODO: cambiar esta cosa por archivo script
...
La **modelización más simple de estos problemas  es la llamada distribución geométrica**.
...
Notemos que si $k=0,1,2,\ldots$ el límite de a función de distribución es 
...
Recordemos que $P(X=x)=(1-p)^x\cdot p$ si $x=0,10,2,\ldots$ y 
aplicado la fŕomula anterior con $r=1-p$
...
<div class="example"> **Ejemplo**: Una puerta varias llaves y una cerradura
...
## Ejemplo: Madrid/Barça, Barça/Madrid,...
...
La desviación típica es $\sqrt{`r round((1- (72/178))/(72/178)^2,4)`}=`r round(sqrt((1- (72/178))/(72/178)^2),4)`.$
...
Veamos pues los cálculos para la $Geom(p)$ que empieza en $0$.
...
Qué probabilidades son las que calcula el siguiente código
...
qué tipo de variables geométricas son. 

</div>

## Cálculos con python  esperanza y  varianza

Con python también podemos pedir algunos parámetros asociado a una función de distribución predefinida
...
Supongamos que disponemos de 10 llaves distintas y tenemos que abrir una puerta con **dos cerraduras**  empezamos por la primera cerradura  y cada vez olvidamos que llave que hemos probado, una vez abierta la primera cerradura probamos con la segunda hasta que también la abrimos.
...
Una v.a. con este tipo de distribución recibe el nombre de *binomial negativa* y la denotaremos por $BN(p,r)$. 

Notemos que $BN(p,1)=Ge(p)$.
...
${-6\choose 4}=\frac{-6\cdot (-6-1)\cdot \cdot (-6-2)\cdot (-9-3) }{4!}=\frac{-6\cdot(-7)\cdot (-8)\cdot (-9)}{24}=\frac{3024}{24}=126.$
...
## Esperanza y varianza de una $BN(r,p)$

Su **esperanza es**

$E(X)=\displaystyle\sum_{x=r}^{+\infty} x\cdot {x-1\choose r-1} \cdot (1-p)^{x-r}\cdot p^r=\frac{r}{p}.$


La esperanza de $X^2$

$E(X^2)=\displaystyle\sum_{x=r}^{+\infty} x^2\cdot {x-1\choose r-1} \cdot (1-p)^{x-r}\cdot p^r=\frac{r\cdot (1-p)}{p^2}+\left(\frac{r}{p}\right)^2$

Por último la varianza es 

$Var(X)=E(X^2)-E(X)^2=\frac{r\cdot (1-p)}{p^2}.$
...
--------------:|:------------
$D_X=$ | $\{r,1,2,3\ldots\}$  
$P_X(x)=P(X=x)=$ | $\left\{\begin{array}{ll} {x-1\choose r-1} \cdot (1-p)^{x-r}\cdot p^r & \mbox{si }  x=r,r+1,\ldots \\ 0 & \mbox{en otro caso.}\end{array}\right.$
$F_X(x)=P(X\leq X)=$ | $\displaystyle\sum_{k=r}^x P(X=k)$.  Calcular al suma o utilizar funciones de R o python.
$E(X)=\frac{r}{p}$ | $Var(X)=\frac{r\cdot (1-p)}{p^2}$ 
...
Supongamos que tenemos un sistema que como seguridad que tiene claves  de  2 dígitos  $00,01,\ldots 99$.  
En total 100 posibilidades.

Como dos dígitos es poco  ponemos tres fases de acceso al sistema cada una de 2 dígitos. Hay que averiguar las tres claves de forma consecutiva para entrar en el sistema
...
Sea $X$ la v.a que nos da el número de intentos necesarios para entrar en el sistema.
...
1. ¿Cuál es la distribución de probabilidad de $X$ la v.a que nos da el número de intentos necesarios para entrar en el sistema.
2. ¿Cuál es la función de probabilidad y de distribución del $X$
3. ¿Cuál es la probabilidad de necesitar exactamente 150 intentos para entrar en el sistema?
4. ¿Cuál es la probabilidad de necesitar más de  5  intentos para entrar en el sistema?
5. ¿Cuál es  el número esperado de intentos para entrar en el sistema? ¿Y su desviación típica?
...
**Solución 1.** ¿Cuál es la distribución de probabilidad de $X$ la v.a que nos da el número de intentos necesarios para entrar en el sistema.


Bajo estados condiciones tenemos que la probabilidad de "éxito" de cada inteton es $p=\frac{1}{100}=.01$. Y como cada vez *olvidamos* en los dígitos cada intento será inpedendiente del anterior.

Así que  la variable $X$ que queremos modelar  cuenta el número de intentos independientes hasta conseguir 3 éxitos en un experimento $Ber(p=0.01)$ por lo tantop $X$ sigue un distribución $BN(r=3,p=0.01).$
...
**Solución 2.**  ¿Cuál es la distribución de probabilidad de $X$ la v.a que nos da el número de intentos necesarios para entrar en el sistema.

En general la   función de probabilidad  de una $BN(r,p)$ es 
...
\left\{
\begin{array}{cc} 
{x-1\choose r-1} \cdot (1-p)^{x-r}\cdot p^r & \mbox{si }  x=r,r+1,\ldots \\ 0 & \mbox{en otro caso.}\end{array}\right.
$$
En particular la   función de probabilidad  de una $BN(r=3,p=0.01)$ es
...
\left\{
\begin{array}{cc} 
{x-1\choose 2} \cdot (0.99)^{x-3}\cdot 0.01^3 & \mbox{si }  x=3,4,\ldots \\ 0 & \mbox{en otro caso.}\end{array}\right.
$$
...
## Solución  ejemplo  $BN(r,p)$ 


<div class="example-sol">

**Solución 3.** ¿Cuál es la probabilidad de necesitar exactamente 150 intentos para entrar en el sistema?
...
$$
P(X=150)= {149\choose 2} \cdot (0.99)^{147}\cdot 0.01^3
$$

Lo calcularemos con R

```{r}
choose(149,2)*0.99^147*0.01^3
```
</div>
...
**Solución 4.** ¿Cuál es la probabilidad de necesitar más de  5  intentos para entrar en el sistema?


$$P(X>5)=1-P(X\leq 5)$$

Calculemos P(X\leq 5)

$$
\begin{eqnarray*}
P(X\leq 5) &=& P(X=3)+P(X=4)+P(X=5)= \sum_{k=3}^5 {k-1\choose 2} \cdot (0.99)^{k-3}\cdot 0.01^3\\
&=&
 {3-1\choose 2} \cdot(0.99)^{3-3}\cdot 0.01^3\\
&+&
 {4-1\choose 2} \cdot (0.99)^{4-3}\cdot 0.01^3\\
&+&
 {5-1\choose 2} \cdot (0.99)^{5-3}\cdot 0.01^3=\ldots
\end{eqnarray*}
$$
```{r}
choose(2,2)*0.99^0*0.01*3+choose(3,2)*0.99^1*0.01*3+choose(4,2)*0.99^2*0.01*3
```
...
**Solución 5.** ¿Cuál es  el número esperado de intentos para entrar en el sistema? ¿Y su desviación típica?
...
$F_X(x)=P(X\leq X)=$ |  Función de R, python  o tabulada  
$E(X)=\lambda$ | $Var(X)=\lambda$


## Aproximación de la distribución binomial por la Poisson

Bajo el punto de vista anterior y si $p$ es pequeño y $n$ suficientemente grande (existen distintos criterios por ejemplo $n>20$ ó $30$ y  $p\leq 0.1$) podemos aproximar una $B(n,p)$ por una $Po(n\cdot p)$
