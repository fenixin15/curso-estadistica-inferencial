...
plot(x=c(0,1),y=dbinom(c(0,1),size=1,prob=0.25),
    ...
    main=paste0(c("Función de probailidad\n Ber(p=",p,")"),collapse="")
...
* O también tirar una pelota a una canasta de balocesto hasta obtener la primer la canasta.

* Desde otro punto de vista también podemos intentar modelar el número de veces que accionamos una interreptor  y la bombilla se ilumina  hasta que falla. 
...
* la media se incrementa en u intento el éxito  $E(Y)=E(X+1)=E(X)+1=\frac{1-p}{p}+1=\frac1{p}$.
* La varianza es la misma $Var(Y)=Var(X+1)=Var(X)=\frac{1-p}{p^2}$.


## Resumen $Ge(p)$ comenzando en $1$.

$Y$ geometrica  (que cuenta el éxito empieza en 1)| número de INTENTOS  para OBTENER el primer éxito
------:|:-----
...
La desciación típica es $\sqrt{`r round((1- (72/178))/(72/178)^2,4)`}=`r round(sqrt((1- (72/178))/(72/178)^2),4)`.$
...
El  código de la siguiente página dibuja las función de probabilidad y la de distribución de una  $Ge(p=0.25)$
...
\end{array}
\right)\cdot
(1-p)^{x-r}\cdot p^r & \mbox{si }  x=r,r+1,\ldots \\
0 & \mbox{en otro caso.}
\end{array}
\right.$
