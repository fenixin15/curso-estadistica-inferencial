...
$$P\left(X\geq k+j\big| X> j\right)=P(X\geq k)$$
...
$$
P(X\geq k)=1-P(X< k)=1-P(X< k-1)=1-\left(1-(1-p)^{k-1+1}\right)=(1-p)^k
$$
...
$$
\begin{eqnarray*}
P\left(X> k+j\big| X> j\right)&=&\frac{P\left(\{X> k+j\}\cap \{X>j\} \right)}{P\left(X>j\right)}=
\frac{P\left(X>k+j \right)}{P\left(X> j \right)}=
\frac{1-P(X\leq k+j)}{1-P(X\leq j)}\\
&=& \frac{1-P(X\leq k+j)}{1-P(X\leq j)}=
\frac{1-(1-(1-p)^{k+j+1})}{1-(1-(1-p)^{j+1})}=\frac{(1-p)^{k+j+1}}{(1-p)^{j+1}}=(1-p)^{k}
\end{eqnarray*}
$$
...
Es decir después de 5 fracasos no estamos más cerca de abrir la puerta, ya que  por la propiedad de la  falta de  memoria  es como si empezásemos  de nuevo a abrir la puerta.
</div>
