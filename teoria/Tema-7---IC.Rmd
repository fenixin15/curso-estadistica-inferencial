...
Por ejemplo, para buscar  un intervalo de confianza  $]A,B[$ del $95$%, buscaremos $A,B$ de manera que 
$$
...
## Intervalos de confianza para el parámetro $\mu$ de una población normal con $\sigma$ conocida. Simulación
...
\left]518-`r round(qt(0.95,23),3)` \frac{40}{\sqrt{24}},
518 + `r round(qt(0.95,23),3)` \frac{40}{\sqrt{24}}\right[ = 
\left] `r round(518-qt(0.95,23)*40/sqrt(24),3)`, `r round(518+qt(0.95,23)*40/sqrt(24),3)`\right[.
$$
Observamos que no contiene a 500 (¡pero se equivoca a favor del consumidor!)
</div>
...
## Intervalos de confianza para el parámetro $\mu$ de una población cualquiera con $\sigma$ desconocida y tamaño muestral grande. Estimación del tamaño de la muestra con una amplitud prefijada
...
```
donde  `conf.level` es el nivel de confianza $1-\alpha$ en tanto por uno. Valor por defecto: $1-\alpha =0.95$.
...
## Método de Wilson
<l class="definition">Definición.</l>
En estas  condiciones, un intervalo  de confianza  del $(1-\alpha)\cdot 100\%$ I.C. para $p$ es  (donde $\widehat{q}_{X}=1-\widehat{p}_{X}$) es:
$$
\left]\frac{\widehat{p}_{X}+\frac{z_{1-{\alpha}/{2}}^2}{2n}-z_{1-{\alpha}/{2}}\sqrt{\frac{\widehat{p}_{X}\widehat{q}_{X}}{n}+\frac{z_{1-{\alpha}/{2}}^2}{4n^2}}}{1+\frac{z_{1-{\alpha}/{2}}^2}{n}}\right.,
 \left.\frac{\widehat{p}_{X}+\frac{z_{1-{\alpha}/{2}}^2}{2n}+z_{1-{\alpha}/{2}}\sqrt{\frac{\widehat{p}_{X}\widehat{q}_{X}}{n}+\frac{z_{1-{\alpha}/{2}}^2}{4n^2}}}{1+\frac{z_{1-{\alpha}/{2}}^2}{n}}\right[
$$
