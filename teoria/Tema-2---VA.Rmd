...
* La variables aleatorias **continuas**  toman  valores en intervalos.
* Variables aleatorias **mixtas**; con una parte discreta y otra continua.
...
Si $X$ no asume ese valor entonces
$P_{X}(x)=0$.
</div>
...
Supongamos que el dado está bien balanceado. 
...
Poedmos reescribir la función de probabilidad de $X$ de forma simplificada:
...
Efectivamente los valores de la función de distribuciḉon suman 1
...
Tenemos que  $\overline{\left\{X>x\right\}}=\left\{X\leq x\right\}$. Además

$$P(X>x)=1-P(\overline{\left\{X>x\right\}})=1-P(X\leq x)=1-F_{X}(x)$$
    
lo que demuestra la primera propiedad

El complementario de mayor que $x$ es $\left\{a< X \leq b\right\}= \left\{X\leq b\right\}-\left\{X\leq  a\right\}$
...
Este valor es el que "*esperamos*" que se resuma la v.a. o esperamos que las
...
$$E(X)=\displaystyle \lim_{n\to\infty}\sum_{x=1}^6x \frac{n_{x}}{n}.$$
...
<l class="prop"> **Conceptos básicos de las  series geométricas y 1**</l>
...
<l class="prop"> **Conceptos básicos de las  series geométricas y 2** </l>
...
<l class="prop"> **Conceptos básicos de las  series geométricas y 3** </l>
...
* Llamaremos  *momento de orden $r$* respecto al punto $C$ a $E\left((X-C)^m\right)$.
* Cuando $C=0$ los momentos reciben el nombre de *momentos respecto al origen*.
* Cuando $C=E(X)$ reciben el nombre de *momentos centrales o respecto de la media*Luego la esperanza es el momento de orden $1$ respecto al origen. 
...
## Medidad de la variabilidad: la varianza
...
Un **cambio de variable lineal** o **tran^sformación lineal**
...
$P(X=x_0)=F_X(x_0)-F(x_0^{-})=0$. Por lo que no tiene sentido definir `'\textit{función de
probabilidad}".
...
## Ejemplo: Distribución uniforme en el intervalo unidad.

<div class="example"> **Ejemplo: distancia el dardo centro** </l>
Supongamos que lanzamos un dardo a una diana de radio $1$, de forma que sea
"*equiprobable*" cualquier distancia al centro (!Cuidado! esto no es equivalente
...
* C.F. "*longitud favorable*" $x-0$.
* C.P. "*longitud posible*" $1-0$.
* Luego $P(X\leq x)=\frac{C.F.}{C.P.}=\frac{x-0}{1-0}=x$.


## Gráfica de la función de distribución uniforme

"Función de distribución de una v.a. uniforme en el intervalo unidad."
...
La primera es evidente  $P(X<b)=P(X<b)+P(X=b)=P(X<b)$

Para demostra la segunda

$\{X<a\}\cap \{a<X<b\}=\emptyset$ $\{X<a\}\cup \{a<X<b\}=\{X<a\}$ entonces
...
La demostracñon de la tercera propiedad es similar a laa segunda  pero aplicando la primera.
...
<div class="exercice"> Se deja la demostración como ejercicio </div>


## Ejemplo 

<l class="exercice"> **Ejemplo: lanzamiento de dardos**</l>
...
<l class="prop">**Propiedades de la función de densidad 1** </l> 
...
<l class="prop">**Propiedades de la función de densidad 2** </l>
...
<div  class="exercise"> **Ejercicio**: Comprobar estas propiedades en la diana. </div>
...
si tarda más el proceso se cancela.
...
* $\sigma_{X}^2\geq 0$
* $Var(cte)=E(cte^2)-(E(cte))^2= cte^2 - cte^2=0$
* $Var(x)=E(X^2)-\mu_{X}^2=\int\limits_{-\infty}^{+\infty}x^2
f_{X}(x)dx - \mu_{X}^2.$
...
5 &euro; y el beneficio por litro es de 10 pts por botella.
Definimos $T=10 X-50000$ que será el beneficio después de gastos.
</div>

<div class="example-sol"> 

Entonces la esperanza del neneficio es 
...
<l class="prop"> **Propiedad: Tranformación de v.a. discretas </l>
...
transformación de una v.a. continua puede ser continua, discreta, mixta...

## Propiedades

Desafortunadamente para variables no discetas el asunto  no es tan sencillo como el anterior, pues la
transformación de, por ejemplo, una v.a. continua puede ser continua, discreta, mixta...

<l class="prop"> **Propiedad: Transformación de v.a. continuas en continuas** </l>
...
<l class="prop"> **Propiedad: Densidad de una transformación de una v.a. continua**</l>
...
<l class="prop"> **Desigualda de Markov**</l>

Sea $X$ una v.a. positiva con $E(X)$ finita.
Entonces 
...
<div class="observ"> **Utilidad básica de la desigualda de Chebychev**</div>
...
Chebychev para distintos valores de $a>0$ tenemos la siguiente tabla.
...
* Por ejemplo para $a=2$ esta desigualdad se puede interpretar como 
