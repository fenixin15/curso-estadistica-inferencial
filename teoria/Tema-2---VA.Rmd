...
Calculemos en  el ejemplo anterior la varianza del número de errores. Recordemos que:
...
Un **cambio de variable lineal** o **transformación lineal**
...
*[a)] $P(X<b)=P(X<b)+P(X=b)=P(X<b)$
*[c)] \'Idem que b) aplicando a).
...
<l class="exercice"> ** Ejemplo juego de lanzamiento de dardos**</l>
...
$$F_X(x)=\int_{-\infty}^{x} f_X(t) dt.\mbox{ para todo } x\in\mathbb{R}.$$
...
* Si $x \leq 0$ entonces $\int_{-\infty}^x f_X(t) dt = 0.$
*  Si $0\leq x\leq 1$ entonces $\int_{-\infty}^x f_X(t) dt =
\int_0^x 1 dt = x.$
* Si $x\geq 1$  entonces $\int_{-\infty}^x f_X(t) dt =
\int_0^1 1 dt = 1.$


Por lo tanto  $F_X(x)=\int_{-\infty}^x f_X(t) dt$ para todo $x\in\mathbb{R}.$
...
\end{split}
\end{equation*}
* Si $A$ es un conjunto  adecuado de $\mathbb{R}$
...
* A $\sigma_{X}=+\sqrt{\sigma_{X}^2}$ se le denomina desviación típica  de $X$.
...
Resultado $\mu_{X}=\frac12$, $E(X^2)=\frac13$,
$Var(X)=\frac1{12}$.
...
$X(\Omega)=\{x_1,x_2,\ldots,x_{n},..\}$ y sea $h:\mathbb{R}\to\mathbb{R}$ una aplicación.
...
## Desigualdades de Markov y de Chebyshef
* Veremos en esta sección distintas desigualdades que acotan determinadas probabilidades de
una variable aleatoria.
* Estas desigualdades sirven en algunos casos para acotar
probabilidades de determinados sucesos.
* También son útiles desde el punto de vista
...
\begin{equation*}
\begin{split}
E(X)=& \int_{-\infty}^{+\infty} x f_{X}(x) dx=  \int_0^{+\infty} x f_{X}(x) dx\\
= & \displaystyle \int_0^{a} x f_{X}(x)
