---
title: "Tema 3 - Variables Aleatorias. Complementos"
...
P_X(0) & = \frac{1}{2\pi}\int_0^{2\pi} \left(1+p\left(\mathrm{e}^{\mathrm{i}w} -1 \right)\right) e^{-\mathrm{i}w\cdot 0}\, dw =\frac{1}{2\pi}\left(\int_0^{2\pi} (1-p) \, dw + \int_0^{2\pi} p \mathrm{e}^{\mathrm{i}w}\, dw\right) \\ & = \frac{1}{2\pi}\left( (1-p)  +p \left[\frac{\mathrm{e}^{\mathrm{i}w}}{\mathrm{i}}\right]_0^{2\pi}\right)=\frac{1}{2\pi}\left((1-p) + p\cdot 0\right)=1-p.
\end{array}
$$
</div>
...
$$
O sea, el momento de orden $n$ de $X$ es la derivada $n$-ésima de la función generatriz de momentos evaluada en $w=0$ dividido por $\mathrm{i}^n$.
...
hist(muestra,freq=FALSE,ylab="frecuencias relativas")
x2=seq(from=0,to=2,by=0.01)
lines(x2,f(x2),col="red")
```
...
=======
Veremos cómo la **entropía** cuantifica la **incertidumbre** por la cantidad de **información** requerida para especificar el resultado de un experimento aleatorio. 
>>>>>>> 2bf3a35820b3c3811586ce08f3d0f408bbac9e25
