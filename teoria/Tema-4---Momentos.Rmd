...
=======
>>>>>>> 6ada5eab3a8b4f9ef21856806a2864ad96b3a9d5
...
Definimos la función generatriz de momentos $\phi_X(t)$ en el valor $t$ como: $\phi_X(t)=E\left(\mathrm{e}^{tX}\right).$
...
$$
\phi_X (t)=E\left(\mathrm{e}^{tX}\right) =p\mathrm{e}^{t\cdot 1}+(1-p)\mathrm{e}^{t\cdot 0}=p\mathrm{e}^t+(1-p)=1+p\left(\mathrm{e}^t -1 \right).
$$
</div>
...
Su función generatriz de momentos será:
$$
\phi_X (t)=E\left(\mathrm{e}^{tX}\right)=\int_0^\infty \mathrm{e}^{t x}\lambda \mathrm{e}^{-\lambda x}\, dx = \lambda \int_0^\infty\mathrm{e}^{(t-\lambda)x}\, dx = \lambda\left[\frac{\mathrm{e}^{(t-\lambda)x}}{t-\lambda}\right]_{x=0}^{x=\infty} = \frac{\lambda}{\lambda -t},\ \mbox{si } t<\lambda. 
$$
En este caso vemos que el dominio de la función generatriz de momentos $\phi_X$ es $(-\infty,\lambda)$, ya que si $t\geq \lambda$, la integral anterior no es convergente.

Fijémonos por lo que vendrá más adelante que, como $\lambda >0$, el valor $0$ pertenece al dominio de $\phi_X$.
...
<l class="prop"> Proposición </l>
Sean $X$ una variable aleatoria con **función generatriz de momentos** $\phi_X(t)$. Entonces, el momento de orden $n$ de $X$ se puede obtener de la forma siguiente:
$$
\momento_n =E\left(X^n\right)=\frac{d}{d t^n}\phi_X(t)|_{t=0} =\phi_X^{(n)}(0).
$$
...
Recordemos la definición de la función generatriz de momentos: $\phi_X(t)=E\left(\mathrm{e}^{tX}\right).$

La idea de la demostración es probar por inducción que $\phi_X^{(n)}(t) =E\left(\mathrm{e}^{tX}\cdot X^n\right)$.

Veámoslo para $n=1$: $\phi_X'(t)=E\left(\mathrm{e}^{tX}\cdot X\right)$. 

Seguidamente, apliquemos inducción sobre $n$. Supongamos que $\phi_X^{(n)}(t) =E\left(\mathrm{e}^{tX}\cdot X^n\right)$ y veamos que $\phi_X^{(n+1)}(t) =E\left(\mathrm{e}^{tX}\cdot X^{n+1}\right)$:
$\phi_X^{(n+1)}(t) =\frac{d}{dt}(\phi_X^{(n)}(t)) =\frac{d}{dt}E\left(\mathrm{e}^{tX}\cdot X^n\right) = E\left(\mathrm{e}^{tX}\cdot X^{n+1}\right),$
tal como queríamos demostrar.

Ahora si aplicamos la expresión demostrada $\phi_X^{(n)}(t) =E\left(\mathrm{e}^{tX}\cdot X^n\right)$ a $t=0$, obtenemos:
$\phi_X^{(n)}(0) =E\left(X^n\right)=\momento_n,$
...
En este caso, recordemos que: $\phi_X (t)=1+p\left(\mathrm{e}^t -1 \right).$

Se puede comprobar que $\phi_X^{(n)}(t)=p\mathrm{e}^t$. Por tanto:
$$
\momento_n = \phi_X^{(n)}(0)=p,
$$
...
En este caso, recordemos que: $\phi_X (t)=\frac{\lambda}{\lambda -t},$ para $t<\lambda$ pero como $\lambda >0$, $t=0$ cumple la expresión anterior.

Dejamos como ejercicio para el lector comprobar que: $\phi_X^{(n)}(t)=\frac{\lambda n!}{(\lambda-t)^{n+1}}$.

Por tanto: 
$$
\momento_n = \phi_X^{(n)}(0) = \frac{\lambda n!}{\lambda^{n+1}}=\frac{n!}{\lambda^n},
$$
