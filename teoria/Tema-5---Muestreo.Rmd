...
<div class="example">
Si quisiéramos una muestra de 10 flores sin reposición, haríamos los pasos siguientes:
</div>
...
Seguidamente, hallamos la etiqueta de la primera flor
```{r}
...
En primer lugar, fijamos la semilla de aleatoriedad:
```{r}
set.seed(25) 
```
...
```
## Muestreo aleatorio estratificado {-}
seguidamente, calculamos la muestra estratificada juntando las tres muestras de cada especie
```{r}
...
Por último, mostramos la muestra: 
```{r}
muestra.iris.est
```
...
Y finalmente los mostramos por pantalla: (mostramos sólo los 8 primeros)
