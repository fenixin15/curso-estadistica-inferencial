...
```{r}
set.seed(15)
(primera.flor=sample(1:150,1)) # hallamos la etiqueta de la primera flor
incremento = floor(150/10) # hallamos el incremento que vamos a ir
# sumando a la primera etiqueta que hemos elegido
...
muestra.iris.10.sis = iris[flores.elegidas.10.sis,] # calculamos la subtabla 
# de las flores de la muestra
```
</example>
...
<div class=example>
```{r}
muestra.iris.10.sis # mostrarmos la subtabla de la muestra
```
</example>
...
<div class=example>
Vamos a considerar que la tabla de datos iris está estratificada según tres estratos. Cada estrato está compuesto por las 50 flores de la misma especie. Vamos a hallar una muestra de tamaño 15 hallando tres muestras de tamaño 5 de cada especie (estrato) con reposición y después juntaremos la tres submuestras.
```{r}
set.seed(25) # fijamos la semilla de aleatoriedad
fls.muestra.setosa=sample(1:50,5,replace=TRUE) # hallamos las flores de la especie setosa
fls.muestra.versicolor=sample(51:100,5,replace=TRUE) # idem con versicolor
fls.muestra.virginica=sample(101:150,5,replace=TRUE) # idem con virginica
muestra.iris.10.est=rbind(iris[fls.muestra.setosa,],iris[fls.muestra.versicolor,],
...
<div class=example>
```{r}
muestra.iris.10.est # mostramos la muestra
```
</example>
