...
* De la variabilidad del estimador


* Del tamaño de la muestra


* Del **nivel de confianza** de la  estimación: cuan seguros  queremos estar de que la estimación es correcta
...
\begin{quote}
Con 95\% de confianza podemos afirmar que entre un 73.1\% y un 80.1\% de los estudiantes de la UIB aceptan \ldots
\end{quote}
\end{frame}
...
En la Encuesta de Población Activa} (\blue{EPA):
...
EPA de octubre de 2013: \smallskip
\begin{itemize}
* El número  estimado de parados a nivel nacional fue del  5\,904\,700 \smallskip

* El error de muestreo fue de un 0.87\% \smallskip

* Por lo tanto, estamos bastante seguros (nivel de confianza del 95\%) de que el número de parados estaba entre
$$
\begin{array}{rl}
 ...
 * La EPA de junio del 2013 había estimado el número de parados  en  5\,977\,500
 \smallskip
 
 * No hay evidencia de que  el paro bajase
...
<div class="example">
**Ejemplo** En un experimento en el que se ha medido la tasa oficial  de  alcoholemia en sangre a 40 varones (sobrios) después de tomar 3 cañas de cerveza de 330 ml.
...
Dado un parámetro $\theta$, el intervalo $]A,B[$ es un **intervalo de confianza** del
...
<div class="example">
**Ejemplo** $]A,B[$ es un intervalo de confianza del $95\%$ (o de nivel de significación de 0.05) si
$$
P(A<\theta<B)=0.95
$$
\end{frame}
...
**Por defecto}, buscaremos  intervalos bilaterales tales que la \emph{cola** de probabilidad sobrante $\alpha$ se reparta por igual a cada lado  del intervalo:
$$
...
<div class="example">
**Ejemplo** Para buscar  un intervalo de confianza  $]A,B[$ del $95\%$, buscaremos $A,B$ de manera que 
$$
...
\begin{frame}
## <div class="example">
**Ejemplo** $\mu$ de población normal con $\sigma$ conocida
...
Además tenemos que:
\begin{itemize}
* Está centrado en $\overline{X}$


* El 0.025 de probabilidad restante  está repartido por igual en los dos extremos del intervalo
...
* Para una muestra concreta: La probabilidad de que la media  $\mu$ de la población que ha producido esta muestra, esté en este intervalo concreto, es del 97.5\%


* En ocasiones lo entenderemos como:  ``La probabilidad de que $\mu$ esté en este intervalo es del 97.5\%''


* Pero la frase anterior es mentira} (\emph{es un abuso de lenguaje): La $\mu$ concreta es un valor fijo, por lo  tanto  que pertenezca o   no a este intervalo concreto tiene probabilidad 1 (si   pertenece) y 0 (si no  pertenece) 
...
\begin{frame}
## I.C. para $\mu$ de población normal con $\sigma$ conocida
\vspace*{-3ex}
...
\begin{center}
\begin{tabular}{c|c|c}
confianza  $1-\alpha$} & \blue{Significación $\alpha$} & \blue{$z_{1-\frac{\alpha}{2}}$\\
 \hline
0.900\ & 0.100 &1.64 \\   
0.950 & 0.050 & 1.96\\
0.975 & 0.025 & 2.24 \\
...
<<codigo_plot_intervalos,size="footnotesize",eval=FALSE>>=
...
% \end{center}
<<plot_intervalos,echo=FALSE,out.width="0.9\\linewidth">>=
ICZ=function(x,sigma,alpha){
  c(mean(x)-qnorm(1-alpha/2)*sigma/sqrt(length(x)), 
  mean(x)+qnorm(1-alpha/2)*sigma/sqrt(length(x)))}
set.seed(5)
...
<div class="example">
**Ejemplo** De media, un 5\% de las veces un intervalo de confianza  del 95\% no contendrá el valor real del parámetro
...
\begin{frame}[fragile]
## Ejemplo
Queremos que analizar un sensor que mide la temperatura de un procesador en grados centígrados \footnote{En concreto un **Intel Core i7-2600K}} que tiene 	como temperatura normal de  $32^{\circ}$ a $40^{\circ**$. Para saber si está bien calibrado, diseñamos un experimento en el que ponemos  el procesador el procesador en las mismas condiciones y  tomamos 40 muestras de su temperatura.
Los resultados son los siguientes:

<<temperatura_sensor, size="small">>=
...
\begin{itemize}
* Población normal con $\sigma=\sqrt{1.44}=\Sexpr{sqrt(1.44)}$ conocida
* Muestra aleatoria simplede  tamaño  $n=40$
* Media de la muestra  $\overline{x}=\Sexpr{mean(temperatura)}$
* $1-\alpha=0.9\Rightarrow \alpha=0.1\Rightarrow 1-\frac{\alpha}{2}=0.95$
* $z_{0.95}\approx 1.64$
\begin{itemize}
* Con la tabla de $Z$, $P(Z\leq 1.64)=0.9495\approx 0.95$
* Con R
<<>>=
qnorm(0.95)
@
...
con
$$
\overline{x}=\Sexpr{round(mean(temperatura),3)}, z_{0.95}=1.64, \sigma=\sqrt{1.44}=1.2, n=40
$$

Obtenemos que el intervalo de confianza  del 90\% es 

$$
\Sexpr{mean(temperatura)}\pm 1.64\cdot\frac{1.2}{\sqrt{40}}=
]\Sexpr{round(mean(temperatura)-1.64*1.2/sqrt(4),3)} , \Sexpr{round(mean(temperatura)+1.64*1.2/sqrt(4),3)}[
$$
...
La **amplitud** $A$ de un intervalo de confianza  
$$
\left]\overline{X} -z_{1-\frac{\alpha}{2}} \frac{\sigma}{\sqrt{n}}, \overline{X}+z_{1-\frac{\alpha}{2}}\frac{\sigma}{\sqrt{n}}
\right[
$$
...
$$
\begin{array}{rl}
A}& \displaystyle =\overline{X}+ z_{1-\frac{\alpha}{2}}\frac{\sigma}{\sqrt{n}}-\Big(\overline{X} -z_{1-\frac{\alpha}{2}}\frac{\sigma}{\sqrt{n}}\Big)\\[2ex] & \displaystyle =\red{2\cdot z_{1-\frac{\alpha}{2}}\frac{\sigma}{\sqrt{n}}
\end{array}
$$
%\end{frame}

El **error máximo**, al nivel de confianza  $(1-\alpha)$, que cometemos al estimar $\mu$
...
La **Amplitud** $A$ del intervalo de confianza 
$$
\left]\overline{X} -z_{1-\frac{\alpha}{2}} \frac{\sigma}{\sqrt{n}}, \overline{X}+z_{1-\frac{\alpha}{2}}\frac{\sigma}{\sqrt{n}}
\right[
$$
...
Observaciones
\begin{itemize}
* I.C. para $\mu$ de una población normal con $\sigma$ conocida $n$ y $\alpha$ fijos, si $\sigma$ crece,
 $A$ crece
\smallskip

* I.C. para $\mu$ de población normal con $\sigma$ conocida $\sigma$ y $\alpha$ fijos, si $n$
crece,  $A$ decrece
\smallskip

* I.C. para $\mu$ de población normal con $\sigma$ conocida $\sigma$ y $n$ fijos, si
...
$\widetilde{S}_{X}/\sqrt{n}$}: el **error muestral**, estima el error estándar $\sigma/\sqrt{n$
\end{frame}
...
* La distribución está tabulada (**las tablas en el moodle de la asignatura}), y con R es \texttt{t**.
...
\begin{frame}
## I.C. para $\mu$ de población normal con $\sigma$ desconocida
...
%
%\begin{frame}[fragile]
%## $\mu$ de població normal con $\sigma$ desconocida
%\vspace*{-5ex}
%
%\small
%\begin{verbatim}
...
<<qtimpresora, size="small">>=
qt(0.95,24)
round(qt(0.95,24),2)
@
<<tstudent_intervalo, echo=FALSE>>=
n=24
media=518
s=40
alpha=0.1
qut=round(qt(alpha/2,df = n),2)
a=round(media-qut*s/sqrt(n),3)
b=round(media+qut*s/sqrt(n),3)
@

Operando: $]\Sexpr{a},\Sexpr{b}[$, y no contiene a 500 (¡pero se equivoca a favor del consumidor!)
\end{frame}
...
* Si $n$ es grande $t_{n-1,1-\frac{\alpha}{2}}\approx z_{1-\frac{\alpha}{2}}$ y podemos **aproximarlo** mediante
$$
\left] 
...
\begin{frame}
## I.C. para $\mu$ una población normal con $\sigma$ conocida y  muestra grande

Consideremos la situación siguiente  :
\begin{itemize}
*  $X$ una v.a.  **cualquiera** con media   poblacional $\mu$ desconocida y desv. típ. $\sigma$ conocida
...
\begin{frame}
## I.C. para $\mu$ de población normal con $\sigma$ conocida muestra grande

Consideremos  la situación siguiente  :
\begin{itemize}
*  $X$ una v.a.  **cualquiera} con media   poblacional $\mu$ desconocida  \emph{y desv. típ. $\sigma$ desconocida**

* $X_1,\ldots,X_n$ una m.a.s. de $X$, con media   $\overline{X}$ **y desviación típica muestral $\widetilde{S}_X$**
...
%
%\begin{frame}[fragile]
%## $\mu$ I.C. para $\mu$ de población normal con $\sigma$ conocidamostres grans
%\vspace*{-2ex}
%
%\small
%\begin{verbatim}
...
$$
\overline{x}=\Sexpr{media},\quad \widetilde{s}=\Sexpr{des.tip}
$$
...
\begin{frame}
## Ejemplo
Calculad  un intervalo del que podamos afirmar que con una probabilidad del 95\% contiene la media poblacional de  la tasa de alcoholemia  para hombres después de beber 3 cerveza de 330ml.


Nos piden un **intervalo de confianza  del 95\%** para $\mu$ de una población normal con $\sigma$ conocida  de la v.a. $X$ ``tasa de alcoholemia  para hombres después de beber 3 cervezas de 330ml''
...
$$
\begin{array}{c}
n=40,\overline{x}=\Sexpr{media},\widetilde{s}=\Sexpr{des.tip},\\[1ex]
\alpha=0.05\Rightarrow z_{1-\frac{\alpha}{2}}=z_{0.975}=1.96
\end{array}
$$
%\pause
$$
]\Sexpr{round(media-1.96*des.tip/sqrt(40),3)}, \Sexpr{round(media+1.96*des.tip/sqrt(40),3)}[
$$
...
\Sexpr{round(media-1.96*des.tip/sqrt(40),3)} y 
\Sexpr{round(media+1.96*des.tip/sqrt(40),3)}
...
Para determinar $n$ (grande) que dé cómo  máximo una amplitud $A$ prefijada, necesitamos $\widetilde{S}_X$, que depende de la muestra.
 ...
 Soluciones:
\begin{itemize}
* Si sabemos  la desv. típ. poblacional $\sigma$, la utilizaremos en lugar de  $\widetilde{S}_X$
* Si hemos tomado una muestra previa (**piloto**), emplearemos  la desviación típica muestral para estimar $\sigma$
...
\begin{block}{}
De una población $X$ hemos tomado una  **m.a.s.\ piloto} que ha tenido una  desviación típica muestral $\widetilde{s}_{pilot**$.
...
\begin{itemize}
* Precisión = error máximo = ${A}/{2}=1$


* $\widetilde{s}_{pilot}=10$


* $\alpha=0.01\Rightarrow z_{1-\frac{\alpha}{2}}=z_{0.995}=2.58$
...
Da $]\Sexpr{solución$low},\Sexpr{solución$upper}[$
\end{frame}
...
\begin{frame}
## I.C. para la proporción poblacional muestras grandes I

Consideremos la situación siguiente  :
\begin{itemize}
*  $X$ una v.a. Bernoulli con $p$ desconocida

* $X_1,\ldots,X_n$ una m.a.s. de $X$, con $n$  gran (por Ejemplo, $n\geq 40$) y frecuencia relativa de éxitos $\widehat{p}_{X}$
...
\begin{frame}
## I.C. para la proporción $p$ muestral muestras grandes I
Por lo tanto
...
\begin{itemize}
* [I)] El método de Wilson
* [II)] La solución de Laplace (1812)
...
\begin{frame}[fragile]
## I.C. para la proporción poblacional muestras grandes II

\begin{itemize}
*  $X$ una v.a. Bernoulli con $p$ desconocida
* $X_1,\ldots,X_n$ una m.a.s. de $X$, con $n$  grande y $\widehat{p}_{X}$ alejado  de 0 y 1. Por ejemplo, tal que:
$$
...
En el área de sanidad se emplea  $\Big]0,\dfrac{3}{n}\Big[$ (la regla del 3)
...
## Observaciones 
\begin{itemize}
* El método  de Wilson da un I.C. centrado   en
$$
...
* No se conoce una fórmula para el centro del I.C. de Clopper-Pearson.

* La fórmula de Laplace dóna un I.C.  centrado   en $\widehat{p}_{X}$
...
\Rightarrow
n=\left\lceil\frac{z_{1-\frac{\alpha}{2}}^2}{A_0^2}\right\rceil
$$}
...
y garantizar un error máximo de 0.05, ¿de qué  tamaño  ha de ser la muestra **en el peor de los casos**?
$$
...
\begin{frame}
## I.C. de la varianza $\sigma^2$ de una población normal
...
* $X_1,\ldots,X_n$ una m.a.s. de $X$ y varianza muestral $\widetilde{S}_X^2$
...
\begin{frame}
## I.C. de la varianza $\sigma^2$ de una població normal
...
Observación: el intervalo de confianza  per $\sigma^2$ no está
...
\begin{frame}[fragile]
## Ejemplo
$\displaystyle \left] \frac{(n-1)\widetilde{S}_{X}^2}{\chi_{n-1,1-\frac{\alpha}{2}}^2,
\frac{(n-1)\widetilde{S}_{X}^2}{\chi_{n-1,\frac{\alpha}{2}}^2}
\right[$}
<<tiempo ,size="footnotesize">>=
...
En este caso sí se da el efecto de **población finita** cuando $N$ es relativamente pequeño
...
Consideremos  la situación siguiente  :
\begin{itemize}
*  $X$ una población de  tamaño  $N$ que sigue una distribución con media   poblacional $\mu$ desconocida

* $X_1,\ldots,X_n$ una m.a.\ sin reposición de $X$, con media   $\overline{X}$
...
Consideremos  la situación siguiente  :
\begin{itemize}
*  $X$ una población de  tamaño  $N$ que sigue una distribución Bernoulli con $p$ desconocida

* $X_1,\ldots,X_n$ una m.a.\ sin reposición de $X$, con $n$ muy grande  y con frecuencia relativa de éxitos $\widehat{p}_{X}$ lejos de los  extremos
...
De la población total de estudiantes de grado de la UIB ¿cuántos hemos de escoger de manera aleatoria  y sin  reposición para estimar la proporción de los que han cometido plagio, con un error del 3.52\% y un nivel de confianza  del 95\%?
%\pause
