...
\blue{Recordau que teniu una Taula de Contrastos exhaustiva a Campus Extens}
...
+z_{1-\alpha}\sqrt{\frac{\sigma_1^2}{n_1}+\frac{\sigma_2^2}{n_2}}\right]}
$$}
...
on
$$
f=\left\lfloor\frac{\displaystyle \left( \frac{1.201^2}{40}+\frac{1.579^2}{40}\right)^2}%
{\displaystyle \frac{1}{39}\left(\frac{1.201^2}{40}\right)^2+\frac{1}{39}\left(\frac{1.579^2}{40}\right)^2}\right\rfloor -2=70
$$
...
\emph{Valor que pren}: 
$$
z_0=\frac{9.789-11.385}{\sqrt{\frac{1.201^2}{40}+\frac{1.579^2}{40}}}=-5.0881
$$
\medskip
...
\emph{Decisió final}: Els dos casos han donat el mateix, així que concloem que a $G_1$ tarden menys que a $G_2$
...
\begin{tabular}{c|cc|c|}
  & Éxits & Fracasos &  Total\\\hline
$X_1$ & $n_{11}$ & $n_{12}$ &  $n_{1\bullet}$\\
$X_2$ & $n_{21}$ & $n_{22}$ &  $n_{2\bullet}$ \\\hline 
...
\begin{quote}
\red{En una bossa hi tenim $n_{\bullet1}$ bolles E i $n_{\bullet2}$ bolles F. Probabilitat d'obtenir $n_{11}$ bolles E si en triam $n_{1\bullet}$ de cop.}
\end{quote}
És una hipergeomètrica $H(n_{\bullet1},n_{\bullet2},n_{1\bullet})$. L'empram per calcular els p-valors.
...
\red{p-valor}: $P(H(n_{\bullet1},n_{\bullet2},n_{1\bullet})\geq n_{11})$
\bigskip
...
\red{p-valor}: $P(H(n_{\bullet1},n_{\bullet2},n_{1\bullet})\leq n_{11})$
\bigskip
...
Per determinar si la Sindrome de Mort Sobtada del Nadó (SIDS) té component genètic, es consideren els casos de SIDS en parelles de bessons monozigòtics i dizigòtics. Diguem: 
\begin{itemize}
...
\begin{tabular}{lccc}
\hline
 & \multicolumn{2}{c}{\textbf{Casos de SIDS}} &\\
 \textbf{Tipus de bessons} & Un & Dos & \\
 Monozigòtics & 23 & 1 & 24\\
 Dizigòtics & 35 & 2 & 37\\
  & 58 & 3 & 61
\\\hline
\end{tabular}
...
$$
\begin{array}{rl}
P(H(58,3,24)\geq 23) & \hspace*{-1.5ex}=1-P(H(58,3,24)\leq 22)\\ &  \hspace*{-1.5ex}=\mbox{\texttt{1-phyper(22,58,3,24)}}=0.66
\end{array}$$
\medskip
...
Podem concloure, a un nivell de significació del 95\%, que A produeix malestar general en una proporció dels malalts que és 5 punts percentuals superior  a la proporció dels malalts en què el produeix B?
...
\emph{$p$-valor}: 
$2\cdot P(t_{59}\geq 6.37)=2\cdot 10^{-8}$ molt petit
\medskip
...
Al cap d'un temps se'ls subministra als mateixos individus un placebo
...
L'estadístic de contrast el valor $z_0=2.4$
\medskip
