...
{Valor que pren}: $z_0=\dfrac{9.789-11.385}{\sqrt{\frac{1}{40}+\frac{4}{40}}}=-4.514$
...
\begin{enumerate}
[(1)] Suposam que $\sigma_1=\sigma_2$
[(2)] Suposam que $\sigma_1\neq \sigma_2$
\end{enumerate}
...
\begin{quote}
{En una bossa hi tenim $n_{1\bullet}$ bolles E i $n_{2\bullet}$ bolles F. Probabilitat d'obtenir $n_{11}$ bolles E si en triam $n_{\bullet1}$ de cop.}
\end{quote}
...
{Contrastau la hipòtesi d'igualtat de proporcions al
...
{$H_0:\dfrac{\sigma^2_1}{\sigma_2^2}=1$}
...
La distribució {$F_{n,m}$}, on $n,m$ són els {graus de llibertat},
...
{Contrastau la hipòtesi d'igualtat de variàncies al
...
Es parla en aquest cas de {mostres aparellades} (\textsl{paired})
...
# Contrast per a dues proporcions
