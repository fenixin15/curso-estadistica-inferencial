...
\begin{itemize}
\item \emph{De partició:} Dividim els objectes en un nombre prefixat de clusters; possiblement provam diversos nombres de clusters i ens quedam amb el millor nombre
\end{itemize}
\vspace*{-2ex}
...
\end{itemize}
\begin{center}
\includegraphics[width=0.8 \linewidth]{hclust-example}
\end{center}
...
$$
\|\mathbf{x}\|^2=\sum_{i=1}^n x_i^2\in \RR
$$
...
$$
Aleshores cada $\mathbf{c}_j$ definirà el cluster format pels $\mathbf{x}_i$ que estan més prop d'ell que de cap altre $\mathbf{c}_l$:
$$
...
\begin{enumerate}
\item Escollim $\mathbf{c}_1,\ldots,\mathbf{c}_k$ (com volguem)
\medskip
...
\begin{itemize}
\item No hi ha mètode eficient i universal de triar els centres de partida
...
\vspace*{-2ex}
\begin{center}
\includegraphics[width=0.6 \linewidth]{kink.pdf}
\end{center}
...
\begin{lstlisting}
> dades=matrix(c(0.8,1.3,0.8,1.8, 
1.0,0.9,1.1,0.1,1.1,1.6,1.4,0.6,  
1.5,0.1,2,2.1,1.5,2.3,1.8,1.8,  
2.3,0.5,0.3,2.2,1,2.5,2,0.5, 
2,1.5,2.5,1,0.5,0.5,1,2), 
nrow=18,byrow=TRUE) 
...
> km$tot.withinss #SSC
[1] 7.25375
> km$betweenss  #totss-tot.withins
[1] 9.955694
> 9.955694/17.20944  #between_SS/total_SS
[1] 0.5785019
\end{lstlisting}
\medskip


Aquest darrer valor ja el dóna \texttt{kmeans}: com més petit, millor
\end{frame}
...
\small
\begin{lstlisting}
> kmeans(dades,3,algorithm="Lloyd")
$k$-means clustering with 3 clusters of sizes 6, 5, 7
Cluster means:
       [,1]     [,2]
...
...
> kmeans(dades,3,algorithm="Lloyd")$tot.withinss
[1] 5.965111
\end{lstlisting}
\end{frame}
...
\footnotesize
\begin{lstlisting}
> plot(dades,col=km2$cluste,pch=19,
 main="Optim")
\end{lstlisting}
\vspace*{-3ex}
...
a_2 = \big|\{k\mid x_{ik}=0, x_{jk}=1\}\big| \\[1ex]
a_3 = \big|\{k\mid x_{ik}=1, x_{jk}=0\}\big| \\[1ex]
a_4 = \big|\{k\mid x_{ik}=x_{jk}=0\}\big|
\end{array}
$$
Una semblança entre els objectes $i$ i $j$ es pot definir mitjançant la fórmula genèrica
$$
\red{\sigma_{ij}=\frac{a_1 + \delta a_4}{\alpha a_1 + \beta a_4 +\lambda (a_2 + a_3)}}
$$
\end{frame}
...
Hamming&$1$&$1$&1 & 1& $\dfrac{a_1 + a_4}{n}$\vphantom{$\displaystyle\int_A^A$}\\\hline
Jaccard&$0$&$1$& 1&0 & $\dfrac{a_1}{a_1 + a_2 +a_3}$ \vphantom{$\displaystyle\int_A^A$}\\\hline
Tanimoto&$1$&$2$&1 &1 & $\dfrac{a_1+a_4}{a_1+2(a_2+a_3)+a_4}$\vphantom{$\displaystyle\int_A^A$}\\\hline
Rusell--Rao&0 &1& 1& 1& $\dfrac{a_1}{n}$\vphantom{$\displaystyle\int_A^A$}\\\hline
Diu&$0$&$0.5$& 1& 0& $\dfrac{2a_1}{2a_1 + a_2 +a_3}$\vphantom{$\displaystyle\int_A^A$} \\\hline
Kulczynski&0&1& 0& 0& $\dfrac{a_1}{a_2 + a_3}$\vphantom{$\displaystyle\int_A^A$}\\\hline
\end{tabular}
\end{center}
\end{frame}
...
$$
Se sol prendre com a distància
$$
...
\multicolumn{1}{c}{} & \multicolumn{5}{c}{Planta} & \\
Bosc & A & B & C & D & E & $x_{\bullet j}$ \\ \hline
X & 12 & 3 & 8 & 0 & 24 & 47  \\
Y & 3 & 22 & 15 & 8 & 11  &  59  \\
Z & 0 & 7 & 12 & 20 & 6 & 45 \\\hline
$x_{i\bullet}$ & 15 & 32 & 35 & 28 & 41 & 151\\ \hline 
\end{tabular}
\end{center}
...
En general, conegudes les distàncies $d(C,C_1)$, $d(C,C_2)$ i $d(C_1,C_2)$, hi ha una fórmula genèrica per calcular
$d(C,C_1+ C_2)$:
$$
\begin{array}{rl}
d(C,C_1+C_2)= &\hspace*{-1ex} \delta_1 d(C,C_1)+\delta_2 d(C,C_2)+\delta_3 d(C_1,C_2) \\[1ex] & + \delta_4 |d(C,C_1)-d(C,C_2)|,
\end{array}
$$
...
\hline
Nom&$\delta_1$&$\delta_2$&$\delta_3$&$\delta_4$\\\hline
...
\hline
Centroïde\vphantom{$\displaystyle\int_A^B$} &$\frac{n_{C_1}}{n_{C_1} + n_{C_2}}$&$\frac{n_{C_2}}{n_{C_1} + n_{C_2}}$&$-\frac{n_{C_1} n_{C_2}}{(n_{C_1} + n_{C_2})^2}$&$0$\\\hline
Mediana\vphantom{$\displaystyle\int_A^B$} &$1/2$&$1/2$&$-1/4$&$0$\\\hline
\emph{Ward}\vphantom{$\displaystyle\int_A^B$} &$\frac{n_{C} + n_{C_1}}{n_R + n_{C_1} + n_{C_2}}$&$\frac{n_C + n_{C_2}}{n_C + n_{C_1}+ n_{C_2}}$&$-\frac{n_C}{n_C + n_{C_1} + n_{C_2}}$&$0$\\\hline
\end{tabular}
\end{center}
\end{frame}
...
\begin{center}
\verb?plclust(clust, labels=..., hang=..., ...)?
\end{center}
on
\begin{itemize}
...
\footnotesize
\begin{lstlisting}
> dists=dist(dades,method="euclidean")
> dists
     1      2      3      4      5      6
2 0.3606                                                  
3 0.5000 0.4243                                        
...
> plclust(hclust(dists,method="single"),
 labels=especies)
\end{lstlisting}
\vspace*{-3ex}

\begin{center}
\includegraphics[width=0.75\linewidth]{Rplot1.pdf}
\end{center}
\end{frame}
...
\end{lstlisting}
\begin{center}
\includegraphics[width=0.8 \linewidth]{Rplot2.pdf}
\end{center}
\end{frame}
...
\footnotesize
\begin{lstlisting}
> plclust(hclust(dists,method="average"),
 labels=especies,hang=-1)
\end{lstlisting}
\begin{center}
\includegraphics[width=0.8 \linewidth]{Rplot3.pdf}
\end{center}
\end{frame}
...
\footnotesize
\begin{lstlisting}
> plclust(hclust(dists,method="complete"),
 labels=especies,hang=-1)
\end{lstlisting}
\begin{center}
\includegraphics[width=0.8 \linewidth]{Rplot4.pdf}
\end{center}
\end{frame}
...
\footnotesize
\begin{lstlisting}
> plclust(hclust(dists,method="ward"),
 labels=especies,hang=-1)
\end{lstlisting}
\begin{center}
\includegraphics[width=0.8 \linewidth]{Rplotw.pdf}
\end{center}
\end{frame}
...
\blue{J. Casasnovas, J. Clemente \textsl{et al}, IPMU 2006}
\medskip

Calculàrem la semblança dels camins metabòlics de la Glicòlisi en una sèrie d'organismes    
\begin{center}
\begin{tabular}{l|l|l|l|l|l|l|}
&$\textup{MGE}$&$\textup{HIN}$&$\textup{MTU}$&
$\textup{MJA}$ &$\textup{ECO}$&$\textup{AFU}$\\
\hline
$\textup{MGE}$& 1.00 & 0.33 & 0.07 & 0.02 & 0.17 & 0.22 \\
\hline
$\textup{HIN}$&0.33 & 1.00 & 0.33 & 0.32 & 0.34 & 0.27 \\
\hline
$\textup{MTU}$&0.07 & 0.33 & 1.00 & 0.09 & 0.20 & 0.20 \\
\hline
$\textup{MJA}$&0.02 & 0.32 & 0.09 & 1.00 & 0.18 & 0.24 \\
\hline
$\textup{ECO}$&0.17 & 0.34 & 0.20 & 0.18 & 1.00 & 0.32 \\
\hline
$\textup{AFU}$& 0.22 & 0.27 & 0.20 & 0.24 & 0.32 & 1.00\\
\hline
\end{tabular}
\end{center}
{\footnotesize   AFU: \textit{A.fulgidus};
        MJA: \textit{M.jannaschii};
        MGE: \textit{M.genitalum};
        HIN: \textit{H.influenzae};
        MTU: \textit{M.tuberculosis};
        ECO: \textit{E.coli} }
\end{frame}
