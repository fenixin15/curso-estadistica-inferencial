\documentclass[12pt,spanish,es-nodecimaldot]{article}
\usepackage{babel}
...
<<proporcioninternautas,results='hide',echo=FALSE>>=
phat=0.8
n=100
...
$$\sqrt{\frac{\hat{p}\cdot (1- \hat{p})}{n}}=\sqrt{\frac{\Sexpr{phat}\cdot (\Sexpr{1-phat})}{\Sexpr{n}}}=
\Sexpr{error.estandar}.$$
\else
%\vskip 2cm
\fi
...
\textbf{Solución:}
<<sumasdados,results='hide',echo=FALSE>>=
puntuaciones=1:6
...
$$P(Y=59)= P(100-X\leq 59) = P(X\geq 41)= 1-P(X < 41)=1-P(X\leq 40)=1-0.8962=\Sexpr{1-0.8962}.$$
...
\probl  \textbf{(0.5 puntos)} 
Sea $Z$ una variable aleatoria normal estándar.  Tenemos que \texttt{pnorm(-0.2)}=\Sexpr{pnorm(-0.2)}.
