\SweaveOpts{prefix.string=./dibujos/02/} 
...
<<fig=F,echo=T>>=
options(width = 60)
curve( (1/sqrt(2*pi))*exp(-(1/2)*x^2),from=-4,to =4,ylab=expression(f(x)))
text(c(-3.5,0.3),expression(f(x)==frac(1,sqrt(2 * pi))* e^(-frac(x^2,2))))
@
...
\begin{figure}
<<fig=T,echo=F,width=4.5, height=3.5>>=
curve(2*x^2+1/3,ylab=expression(f(x)), from=0,to=1,axes=F)
axis(1,at=c(-0.1,0,0.25,0.5,0.6,0.75,1),labels=c("",0,0.25,0.5,expression(x),0.75,1))
axis(2,at=c(1/3,1,7/3),labels=c(expression(frac(1,3)),1,expression(k+frac(1,3))))
dx <- seq(0,0.6,length.out=10)
polygon(c(0,dx,0.6),c(0,2*dx^2+1/3,0),border=NA,col="blue")
text(x=0.5,y=2,expression(f(x)==k*x^2+frac(1,3)))
text(x=0.4,y=0.4,expression(F(X)))
lines(c(0,0),c(0,1/3))
lines(c(0.6,0.6),c(0,2*(0.6)^2+1/3))
lines(c(1,1),c(0,(2+1/3)))
lines(seq(-0.2,1,0.1),rep(7/3,times=13),lty=2)
@
...
x <- seq(-4,4,.1)
#par(yaxt=F)
plot(x,dnorm(x),type="l",xlab="",ylab=expression(paste("Densidad  ",f[Z](z))),xaxs="i",yaxs="i",ylim=c(0,.4),bty="l",xaxt="n")
...
#axis(2,at=c(0.2),labels=expression(f[Z](z)))
axis(1,at=c(-1,0,1), labels=c(expression(-z),0,expression(z)))
arrows(0,0.2,-2.5,0.3)
text(-2.5,0.32,expression(F[Z](z)== P(Z<=z) ) )
#dev.copy2eps(eps,filename="normal1.eps",height=300, width=400,bg="white")
#dev.copy2pdf(file="areanormalestandarizquierda.eps",height=4, width=5,bg="white")
@
...
<<fig=T,echo=F,width=6, height=3.8>>=
x <- seq(-4,4,.1)
#par(yaxt=F)
plot(x,dnorm(x),type="l",xlab="",
ylab=expression(paste("Densidad  ",f[Z](z))), 

xaxs="i",yaxs="i",ylim=c(0,.4),bty="l",xaxt="n")
...
axis(1,at=c(-1,0,1), labels=c(expression(-z),0,expression(z)))
 arrows(0,0.2,-2.5,0.3)
 arrows(1.5,0.05,2.5,0.2)
text(-2.5,0.32,expression(F[Z](z)== P(Z<=z) ) )
text(2.8,0.22,expression(F[Z](-z)== P(Z<=-z) ) )
#dev.copy2eps(eps,filename="normal1.eps",height=300, width=400,bg="white")
@	
